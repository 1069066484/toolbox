...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  int64 ms[3], nCh, nDims, pt, pb, pl, pr, flag, k; double *p;
  void *A, *B; mxClassID id; double val=0; char type[1024];
  //mexPrintf("imPadMex mexFunction 1 \n");
  ...
  //mexPrintf("imPadMex mexFunction 2 \n");
  // extract padding amounts
  k = mxGetNumberOfElements(prhs[1]);
  p = (double*) mxGetData(prhs[1]);
  if(k==1) { pt=pb=pl=pr=p[0]; }
  else if (k==2) { pt=pb=p[0]; pl=pr=p[1]; }
  else if (k==4) { pt=p[0]; pb=p[1]; pl=p[2]; pr=p[3]; }
  else mexErrMsgTxt( "Input pad must have 1, 2, or 4 values.");
  //mexPrintf("imPadMex mexFunction 3 \n");
  ...
  //mexPrintf("imPadMex mexFunction 4 \n");
  // create output array
  ms[0]=int64(ns[0])+pt+pb; ms[1]=int64(ns[1])+pl+pr; ms[2]=nCh;
  if( ms[0]<0 || int64(ns[0])<=-pt || int64(ns[0])<=-pb ) ms[0]=0;
  if( ms[1]<0 || int64(ns[1])<=-pl || int64(ns[1])<=-pr ) ms[1]=0;
  plhs[0] = mxCreateNumericArray(3, (const mwSize*) ms, id, mxREAL);
  if( ms[0]==0 || ms[1]==0 ) return;
  //mexPrintf("imPadMex mexFunction 5 \n");
  // pad array
